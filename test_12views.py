...
from tools.test_dataloader_twfview import TestDataloader
import numpy as np
import os
import torch
...
#     for data in labeled_data_loader:
    for data in test_data_loader:
        ...
        pt, img, imgV, img3, img4, img5, img6, img7, img8, img9, img10, img11,img12, centers, corners, normals, neighbor_index, target = data #the last one is the target
        
        pt = Variable(pt).to('cuda')
        img = Variable(img).to('cuda')
        imgV = Variable(imgV).to('cuda')
        img3 = Variable(img3).to('cuda')
        img4 = Variable(img4).to('cuda')
        img5 = Variable(img5).to('cuda')
        img6 = Variable(img6).to('cuda')
        img7 = Variable(img7).to('cuda')
        img8 = Variable(img8).to('cuda')
        img9 = Variable(img9).to('cuda')
        img10 = Variable(img10).to('cuda')
        img11 = Variable(img11).to('cuda')
        img12 = Variable(img12).to('cuda')
        ...
        pt_base = dgcnn(pt)
        img_base = img_net(img, imgV, img3, img4, img5, img6, img7, img8, img9, img10, img11,img12)
...
def extract_features(args):
    ...
    weights_folder = 'ModelNet40_p10_nt_xw2_aw0_cw9_baseshare_Cos_12views_imgsize224_10w'
