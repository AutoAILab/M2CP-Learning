...
#     test_data_loader = torch.utils.data.DataLoader(test_data_set, batch_size= 1, shuffle = False, num_workers=1)
    ...
    labeled_set = TripletDataloader(dataset = 'ModelNet40', num_points = args.num_points, partition='labeled',  perceptange = 100)
...
#     print('length of the dataset: ', len(test_data_set))
    print('length of the dataset: ', len(labeled_set))
...
#     for data in test_data_loader:
        ...
        if np.array(torch.argmax(pt_pred.data.cpu())) != np.array(target.data.cpu())[0]:
            pt_unmatch_lst.append(pt_base.data.cpu())
            mesh_unmatch_lst.append(mesh_base.data.cpu())
            img_unmatch_lst.append(img_base.data.cpu())            
            target_unmatch_lst.append(target.data.cpu())
            
#             pt_unmatch_lst.append(pt_gfeat.data.cpu())
#             mesh_unmatch_lst.append(mesh_gfeat.data.cpu())
#             img_unmatch_lst.append(img_gfeat.data.cpu())            
#             target_unmatch_lst.append(target.data.cpu())
        else:
            pt_correct_lst.append(pt_base.data.cpu())
            mesh_correct_lst.append(mesh_base.data.cpu())
            img_correct_lst.append(img_base.data.cpu())            
            target_correct_lst.append(target.data.cpu())
            
#             pt_correct_lst.append(pt_gfeat.data.cpu())
#             mesh_correct_lst.append(mesh_gfeat.data.cpu())
#             img_correct_lst.append(img_gfeat.data.cpu())            
#             target_correct_lst.append(target.data.cpu())
    ...
    target_unmatch = torch.cat(target_unmatch_lst, dim = 0)
    target_correct = torch.cat(target_correct_lst, dim = 0)
    ...
    target_unmatch_new = target_unmatch.view(-1,1)
    target_correct_new = target_correct.view(-1,1)
    ...
    pt_unmatch = torch.cat(pt_unmatch_lst, dim = 0)
    mesh_unmatch = torch.cat(mesh_unmatch_lst, dim = 0)
    img_unmatch = torch.cat(img_unmatch_lst, dim = 0)
    
    pt_correct = torch.cat(pt_correct_lst, dim = 0)
    mesh_correct = torch.cat(mesh_correct_lst, dim = 0)
    img_correct = torch.cat(img_correct_lst, dim = 0)
...
#     output_img = torch.cat([img_gfeat, target_new], dim=1)
    ...
    output_pt = torch.cat([pt_unmatch, target_unmatch_new], dim=1)
    output_mesh = torch.cat([mesh_unmatch, target_unmatch_new], dim=1)
    output_img = torch.cat([img_unmatch, target_unmatch_new], dim=1)
    print('unmatchshape',output_pt.shape)
    np.save('output_pt_10_unmatch', output_pt)
    np.save('output_mesh_10_unmatch', output_mesh)
    np.save('output_img_10_unmatch', output_img)
    
    output_pt = torch.cat([pt_correct, target_correct_new], dim=1)
    output_mesh = torch.cat([mesh_correct, target_correct_new], dim=1)
    output_img = torch.cat([img_correct, target_correct_new], dim=1)
    print('correctshape',output_pt.shape)
    
    np.save('output_pt_10_correct', output_pt)
    np.save('output_mesh_10_correct', output_mesh)
    np.save('output_img_10_correct', output_img)
...
def extract_features(args):
    ...
    weights_folder = 'ModelNet40-pt1024-mesh-img56-Xentropy-Xcontrast-PointMultiAgreement-T095-Fused-Warmup-2percent_xcenter_p2_nt_xw0_aw0_cw9_grouphshare_Cos'
