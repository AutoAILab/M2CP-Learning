...
#             nn.Linear(256, 256)
    ...
    def forward(self, img, imgV):
        ...
        return img_pred, img_feat, img_base, img_gfeat
...
class Semi3D(nn.Module):
    def __init__(self, img_net, cloud_net, meshnet, fusion_net):
    ...
    def forward(self, pt, img, imgV, centers, corners, normals, neighbor_index):
        ...
        pt_pred, pt_feat, pt_base, pt_gfeat = self.cloud_net(pt)  # [N,C]
        # print(cloud_ris.size(), cloud_zis.size())

        mesh_pred, mesh_feat, mesh_base, mesh_gfeat = self.meshnet(centers, corners, normals, neighbor_index)

        # get the representations and the projections
        img_pred, img_feat, img_base, img_gfeat = self.img_net(img, imgV)  # [N,C]
