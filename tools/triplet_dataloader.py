...
def load_data(partition):
    ...
    DATA_DIR = '/scratch1/zhiminc/MVdata/data/'
    all_data = []
    all_label = []
    img_lst = []
...
#         print(glob.glob(os.path.join(DATA_DIR, 'modelnet40_ply_hdf5_2048', 'ply_data_%s*.h5'%partition)))
        split = h5_name[-4]
        jason_name = '/scratch1/zhiminc/MVdata/data/modelnet40_ply_hdf5_2048/ply_data_' + partition +'_' + split + '_id2file.json'
...
def load_modelnet10_data(partition):
    ...
    for h5_name in glob.glob(os.path.join(DATA_DIR, 'modelnet10_ply_hdf5_2048', 'ply_data_%s*.h5'%partition)):
        split = h5_name[-4]
        jason_name = '/scratch1/zhiminc/MVdata/data/modelnet10_ply_hdf5_2048/ply_data_' + partition +'_' + split + '_id2file.json'
...
#             transforms.Resize(112),
    ...
    def get_mesh(self, item):
        names = self.img_lst[item]
        names = names.split('/')
        mesh_path = '/scratch1/zhiminc/MVdata/data/ModelNet40_Mesh/' + names[0] + '/train/' + names[1][:-4] + '.npz'
    ...
    def __getitem__(self, item):
        ...
        pt1, pt2, target, img, img2 = self.get_data(item)
...
#         return pt1, pt2, img, img2, centers, corners, normals, neighbor_index, target
        return pt1, img, img2, centers, corners, normals, neighbor_index, target
